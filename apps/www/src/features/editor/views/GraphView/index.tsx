...
const GraphCanvas = ({ isWidget }: GraphProps) => {
  ...
  const skipAutoCenter = useGraph(state => state.skipAutoCenter);
  ...
  const onLayoutChange = React.useCallback(
    (layout: ElkRoot) => {
      if (layout.width && layout.height) {
        ...
        const changeRatio =
          Math.abs((areaSize * 100) / (paneWidth * paneHeight)) - 100;
        ...
        setTimeout(() => {
          window.requestAnimationFrame(() => {
            // Only auto-center if not manually centering a node expand
            if ((changeRatio > 70 || isWidget) && !skipAutoCenter) {
              centerView();
            }
            setLoading(false);
          });
    ...
    [isWidget, paneHeight, paneWidth, centerView, setLoading, skipAutoCenter]
  );
...
export const GraphView = ({ isWidget = false }: GraphProps) => {
  ...
  // Initialize graph when JSON changes
  React.useEffect(() => {
    setLoading(true);
    setGraph(json);
  }, [json, setGraph, setLoading]);
